...
void CSVtoASCII_Panel::SetupSpecificOutputSectionItems()
{
	...
	m_ForceSeparators->SetMinSize(FromDIP(wxSize(120, 22)));
	...
	m_AsciiTableStyleChoices.Add("UTF-8 table (Double Outside)");
	m_AsciiTableStyleChoices.Add("UTF-8 table (Single Outside)");
	...
	m_AsciiTableStyle->SetMinSize(FromDIP(wxSize(120, 22)));
	...
	m_CommentType->SetMinSize(FromDIP(wxSize(120, 22)));
...
void CSVtoASCII_Panel::PopulateOutputDataTextBox()
{
	...
	{
		mrt::AsciiTableGenerator<StrType> asciiTableGenerator(m_CSVData);

		mrt::AsciiTableGenerator<StrType>::OStrStream ss;
...
CSVtoASCII_Panel::StrType CSVtoASCII_Panel::GetCommentCharacters()
{
